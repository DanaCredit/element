...
/* harmony default export */ var table_body = ({
  ...
  render: function render(h) {
    ...
    return h(
      ...
      [h('colgroup', [this.columns.map(function (column) {
        ...
        return acc.concat(_this.wrappedRowRender(store, row, acc.length));
      }, []), h('el-tooltip', {
  ...
  methods: {
    ...
    getRowClass: function getRowClass(store, row, rowIndex) {
    ...
    rowRender: function rowRender(store, row, $index, treeRowData) {
      var _this4 = this;
      ...
      var rowClasses = this.getRowClass(store, row, $index);
      var display = true;
      if (treeRowData) {
    ...
    wrappedRowRender: function wrappedRowRender(store, row, $index) {
      var _this5 = this;
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        var tr = this.rowRender(store, row, $index);
        if (!renderExpanded) {
        ...
        return [[tr, h(
          ...
          [h(
            ...
            [renderExpanded(this.$createElement, { row: row, $index: $index, store: store })]
          )]
        )]];
        ...
        var tmp = [this.rowRender(store, row, $index, treeRowData)];
        // 渲染嵌套数据
        if (cur) {
          ...
          var traverse = function traverse(children, parent) {
            ...
            children.forEach(function (node) {
              ...
              i++;
              tmp.push(_this5.rowRender(store, node, $index + i, innerTreeRowData));
              if (cur) {
